...
BPHermo$type<-c(rep("Hermodice_curunculata",length(BPHermo$type)))
BPdfEggnog$type<-c(rep("Microspio_moreii",length(BPdfEggnog$type)))
BPfinal<-rbind(BPHermo[1:8,],BPdfEggnog[1:8,])

BPfinal <-BPfinal[order(BPfinal$`% genes`,decreasing = TRUE),]

# This is to make an space in the xticks
BPfinal[nrow(BPfinal) + 1,] <- c("a",0,"Microspio_moreii")

MFHermo$type<-c(rep("Hermodice_curunculata",length(MFHermo$type)))
MFdfEggnog$type<-c(rep("Microspio_moreii",length(MFdfEggnog$type)))
MFfinal<-rbind(MFHermo[1:8,],MFdfEggnog[1:8,])

MFfinal <-MFfinal[order(MFfinal$`% genes`,decreasing = TRUE),]

CCHermo$type<-c(rep("Hermodice_curunculata",length(CCHermo$type)))
CCdfEggnog$type<-c(rep("Microspio_moreii",length(CCdfEggnog$type)))
CCfinal<-rbind(CCHermo[1:8,],CCdfEggnog[1:8,])

CCfinal <-CCfinal[order(CCfinal$`% genes`,decreasing = TRUE),]

CCfinal[nrow(CCfinal) + 1,] <- c("b",0,"Microspio_moreii")
...
group.colors <- c(Hermodice_curunculata ="#6c6c6c" ,Microspio_moreii="#ececec")

p <-  ggplot(data=allGOterms, aes(x=reorder(Go_terms, row.names(allGOterms)), y=Percentage ,fill=Organism)) +
  geom_bar(position = position_dodge2(width = 0.9, preserve = "single"), stat="identity",inherit.aes=TRUE) +
  labs(fill = "")+
  xlab("Go Terms") +
  ggtitle('Biological process           Cellular components           Molecular function') +
  ylab("Percent of transcripts/genes")+
   theme( legend.background = element_rect(fill = "transparent"),
    legend.box.background = element_rect(fill = "transparent"),
    panel.background = element_rect(fill = "transparent"),
  ...
  scale_x_discrete(breaks=allGOterms$Go_terms[nchar(as.character(allGOterms$Go_terms))!=1])+
...
#guide_legend(label.theme = element_text(angle = 0, face = "italic"))) makes all of the italic
