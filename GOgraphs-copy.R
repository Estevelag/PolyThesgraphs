...
eggnog <- read.csv('transdeceggnog.emapper.annotations', header = FALSE, sep = "\t")
eggnog <- eggnog[-c(1, 2, 3, 4), ]
colnames(eggnog) <-eggnog[c(1),]
eggnog <- eggnog[-c(1), ]
...
TerminBP <- c()
TerminCC <- c()
for (j in 1:length(eggnog$GOs)){
  GoID<- as.list(strsplit(eggnog$GOs[j], ",")[[1]])
...
#BP 181-571 20000
BPHermodicenames<-c('biological_process','metabolic process','cellular process','primary metabolic process','cellular metabolic process','biological regulation','regulation of biological process','macromolecule metabollic process','cellualar component molecule metabolic process','nucleic acid metabolic process','cellular nitrogen component process','biosynthetic process','localization','transport','protein metabolic process')
...
f<-ggplot(CCotherfinal, aes(x=reorder(Goterms, -CCotherfinal$`% genes`), y=CCotherfinal$`% genes`,fill=type)) +
  geom_bar(stat="identity", color="black",width = 0.90, position=position_dodge())+
  xlab("Go Terms") +
...
e<-ggplot(MFotherfinal, aes(x=reorder(Goterms, -MFotherfinal$`% genes`), y=MFotherfinal$`% genes`,fill=type)) +
  geom_bar(stat="identity", color="black",width = 0.90, position=position_dodge())+
  xlab("Go Terms") +
...
#########BP
  ...
  q<-ggplot(BPotherfinal, aes(x=reorder(Goterms, -BPotherfinal$`% genes`), y=BPotherfinal$`% genes`,fill=type)) +
  geom_bar(stat="identity", color="black",width = 0.90, position=position_dodge())+
  xlab("Go Terms") +
